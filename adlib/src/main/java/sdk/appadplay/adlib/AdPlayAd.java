...
//        Volley.newRequestQueue(context).add(request);
        ...
        if (adType.equals(PRE_ROLE_VIDEO)){
            new GetVideoAdProperties().execute(myPublisherId);
        }else if (adType.equals(FULL_SCREEN)){
            new GetVideoAdPropertiesPreroleAd().execute(myPublisherId);
        }
    ...
    public class GetVideoAdPropertiesPreroleAd extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            try {
                ...
                if (!nurl.equals(null) || !nurl.isEmpty() ) {
                    playVideoPreRole(mContext, nurl, videoUrl, playMin, adLayout,adRole,repeat);
                }
    ...
    private void playVideoPreRole(final Context mContext, final String nurl, final String videoUrl, String playMin, final RelativeLayout adLayout, String adRole, String repeat) {


        adLayout.setVisibility(View.GONE);

        final RelativeLayout subLayout = new RelativeLayout(mContext);
        final RelativeLayout.LayoutParams subLayoutParams = new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.MATCH_PARENT, height);

        final TextView txtTimeRemain = new TextView(mContext);
        ...
        final Button btnClose = new Button(mContext);
